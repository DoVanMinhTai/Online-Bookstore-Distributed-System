...
export async function getProductBestSelling(): Promise<ProductBestSeller> {
    const response = await apiClientService.get(ApiRoute.PRODUCT.GET_PRODUCT_BEST_SELLING);
    return response.json();
}

export async function getProductFeature(): Promise<ProductFeature> {
    const response = await apiClientService.get(ApiRoute.PRODUCT.GET_PRODUCT_FEATURE);
    return response.json();
}
